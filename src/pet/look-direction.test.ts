...
test("按鼠标相对桌宠中心的角度划分十六个方向", () => {
  const bounds = { x: 100, y: 100, width: 200, height: 200 };

  assert.equal(resolveLookDirection({ x: 200, y: 0 }, bounds), "up");
  assert.equal(resolveLookDirection({ x: 400, y: 0 }, bounds), "up-right");
  assert.equal(resolveLookDirection({ x: 400, y: 150 }, bounds), "right-near-up");
  assert.equal(resolveLookDirection({ x: 400, y: 200 }, bounds), "right");
  assert.equal(resolveLookDirection({ x: 400, y: 250 }, bounds), "right-near-down");
